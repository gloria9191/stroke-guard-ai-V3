...
def get_llm_advice(data, prob):
    """LLM 조언 생성"""
    if not GROQ_API_KEY:
        return "혈압과 혈당 관리, 규칙적인 운동을 실천해보세요.<br>금연·절주도 큰 도움이 됩니다."
...
"""
    ...
    return "현재 서버가 혼잡합니다. 혈압·혈당 관리와 규칙적인 운동을 권장드립니다."
...
<div class="hero">
    <div class="hero-content">
        ...
        <p class="subtitle">국내 50만 명 + 미국 NHANES 기반<br>뇌졸중 위험도 예측 AI</p>
        <div>
            <span class="badge">AUC 0.796</span>
            <span class="badge">AI 생활습관 조언</span>
            <span class="badge">실시간 분석</span>
        </div>
    </div>
</div>
    ...
    <div class="row justify-content-center"><div class="col-lg-9">
        <div class="card p-5">
            ...
            <div id="step2" class="d-none text-center">
                <input type="number" class="form-control form-control-lg text-center" id="age" placeholder="나이 (예: 65)">
            </div>

            <div id="step3" class="d-none text-center">
                <input type="number" step="0.1" class="form-control form-control-lg text-center" id="bmi" placeholder="BMI (예: 25.4)">
            </div>

            <div id="step4" class="d-none text-center">
                <input type="number" class="form-control form-control-lg text-center" id="sbp" placeholder="수축기 혈압 (예: 140)">
            </div>

            <div id="step5" class="d-none text-center">
                <input type="number" class="form-control form-control-lg text-center" id="dbp" placeholder="이완기 혈압 (예: 90)">
            </div>

            <div id="step6" class="d-none text-center">
                <input type="number" step="0.1" class="form-control form-control-lg text-center" id="glucose" placeholder="공복 혈당 (예: 120)">
            </div>
...
document.getElementById("submit").onclick = () => {
    ...
    `<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;flex-direction:column">
        ...
        <p class="mt-4 fs-3 text-muted">잠시만 기다려주세요</p>
    </div>`;
...
def predict():
    try:
        ...
        bg = (
            "linear-gradient(135deg,#ff6b6b,#feca57)" if prob > 70 else
        ...
        html = f"""
        <div style="min-height:100vh;background:{bg};color:white;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px;">
            <h1 class="display-1 fw-bold">{prob:.1f}%</h1>
            <h2 class="mt-3">{rt}군</h2>
            <div class="mt-5 fs-3" style="max-width:800px">{advice}</div>
